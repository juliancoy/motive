...
struct OffscreenBlit
{
    ...
    bool initialize(Engine* inEngine, uint32_t width, uint32_t height, const std::array<float, kCurveLutSize>&)
    {
        engine = inEngine;
        extent = {width, height};
        ...
        alloc.memoryTypeIndex = engine->findMemoryType(memReq.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
...
int main(int argc, char** argv)
{
    ...
    std::cout << "[Encode] Engine constructed\n";
    Engine engine;
