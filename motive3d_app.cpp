...
namespace
{
    const std::array<std::filesystem::path, 2> kDefaultVideoCandidates = {
        std::filesystem::path("P1090533_main8_hevc_fast.mkv"),
        std::filesystem::path("..") / "P1090533_main8_hevc_fast.mkv"
    };
    ...
    CommandLineOptions parseCommandLineArgs(int argc, char *argv[])
    {
        ...
        const std::string videoEqualsPrefix = "--video=";
        ...
        for (int i = 1; i < argc; ++i)
        {
            ...
            else if (arg == "--video")
            {
                ...
                {
                    std::cerr << "[CLI] --video requires a file path." << std::endl;
                }
            }
            else if (arg == "--grid")
            {
            ...
            else if (arg == "--disable-culling" || arg == "--disable-cullinh")
            {
            ...
            else if (arg == "--help" || arg == "-h")
            {
                std::cout << "Usage: motive3d [options] [file.gltf/glb]\n"
                          ...
                          << "  --video[=path]     Load video quad scene from file\n"
            ...
            else if (arg.rfind(videoEqualsPrefix, 0) == 0)
            {
                options.videoPath = arg.substr(videoEqualsPrefix.size());
            }
            else if (!arg.empty() && arg[0] != '-')
            {
                const std::filesystem::path positionalPath(arg);
                const std::string ext = positionalPath.extension().string();
                if (ext == ".gltf" || ext == ".glb")
                {
                    options.loadGltf = true;
                    options.gltfPath = positionalPath;
                }
                else if (options.videoPath.empty())
                {
                    options.videoPath = positionalPath;
                }
            }
        }
    ...
    std::filesystem::path resolveVideoPath(const CommandLineOptions& options)
    {
        if (!options.videoPath.empty())
        {
            ...
            std::cerr << "[Video] Provided path does not exist: " << options.videoPath << std::endl;
            return {};
        }

        for (const auto& candidate : kDefaultVideoCandidates)
        {
...
int runMotive3dApp(int argc, char *argv[])
{
    ...
    const std::filesystem::path selectedVideoPath = resolveVideoPath(options);
    if (options.testDecode)
    {
        if (selectedVideoPath.empty())
        {
            std::cerr << "[Video] No decode benchmark input file available." << std::endl;
            return 1;
        }
        return runDecodeBenchmark(selectedVideoPath) ? 0 : 1;
    }
    ...
    else
    {
        ...
        if (selectedVideoPath.empty())
        {
            std::cerr << "[Video] No default sample video found. "
                      << "Use --video <path> to enable video scene bootstrap." << std::endl;
        }
        else
        {
            ...
            if (!video::initializeVideoDecoder(selectedVideoPath, videoState->decoder, playbackParams))
            {
                std::cerr << "[Video] Failed to initialize video decoder for " << selectedVideoPath << std::endl;
                videoState.reset();
            }
        }
        ...
        if (!videoState)
        {
            std::cerr << "[Video] Continuing without video playback scene." << std::endl;
        }
        else
        {
            std::cout << "[Video] Video dimensions: " << videoState->decoder.width << "x" << videoState->decoder.height
                      << ", FPS: " << videoState->decoder.fps << std::endl;
            if (videoState->decoder.width <= 0 || videoState->decoder.height <= 0 || videoState->decoder.bufferSize <= 0)
            {
                std::cerr << "[Video] Decoder returned invalid frame geometry/buffer metadata. "
                          << "Disabling video bootstrap for this run." << std::endl;
                video::cleanupVideoDecoder(videoState->decoder);
                videoState.reset();
            }
        }
